...
{
    char *monter_dir = "c:/work/projects/monter/code/.";
...
//NOTE(chen): added by 4coder base
#if 0
...
#define Insert_Color    0xffff0000
#define Normal_Color    0xff00ff00
#define Transient_Color 0xff0000ff
#define Visual_Color    0xff0000ff
...
CUSTOM_COMMAND_SIG(enter_mode)
{
    ...
    {
        set_theme_colors(app, insert_colors, ArrayCount(insert_colors));
    }
...
CUSTOM_COMMAND_SIG(exit_visual_mode)
{
    ...
    managed_object_free(app, global_editor_state.highlight);
    global_editor_state.highlight = 0;
...
CUSTOM_COMMAND_SIG(visual_wrapper)
{
    ...
    if (global_editor_state.is_in_visual_mode)
    {
        ...
        Managed_Object highlight = global_editor_state.highlight;
        if (!highlight)
        {
            highlight = alloc_buffer_markers_on_buffer(app, buffer.buffer_id, 2, &view_scope);
            global_editor_state.highlight = highlight;
        }
        ...
        if (marker_pos < view.cursor.pos)
        {
            ...
            isearch__update_highlight(app, &view, highlight,
                                      marker_pos, view.cursor.pos);
        }
        else
        {
            //view_set_highlight(app, &view, view.cursor.pos, marker_pos, true);
            isearch__update_highlight(app, &view, highlight,
                                      view.cursor.pos, marker_pos);
        }
...
CUSTOM_COMMAND_SIG(launch_shaderian)
{
    View_Summary active_view = get_active_view(app, AccessAll);
    Buffer_Summary buffer = get_buffer(app, active_view.buffer_id, AccessAll);
    
    char char_buffer[1024] = {};
    snprintf(char_buffer, sizeof(char_buffer), "shaderian %s\n", buffer.file_name);
    String system_command = {};
    system_command.str = char_buffer;
    system_command.size = (int32_t)strlen(char_buffer);
    
    debug_print(system_command.str);
    ...
    String out_buffer = make_fixed_width_string(out_buffer_space);
    exec_system_command(app, 0, buffer_identifier("temp", 4), hot_directory.str, hot_directory.size, system_command.str, system_command.size, CLI_OverlapWithConflict | CLI_CursorAtEnd);
}
...
CUSTOM_COMMAND_SIG(overwrite_line)
{
    kill_line(app);
    enter_mode<mapid_insert>(app);
}

CUSTOM_COMMAND_SIG(swap_header_source_file){
    View_Summary view = get_active_view(app, AccessAll);
...
custom_keys(Bind_Helper *context){
    ...
    bind(context, '2', MDFR_ALT, swap_header_source_file);
    ...
    bind(context, 'C', MDFR_NONE, overwrite_line);
